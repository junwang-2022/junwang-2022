...
proc hpgenselect data=sample_&sub. noprint maxtime=10;
	...
	model &y.=/*cbsa*/ spec_type index_setting index_cm age_grp dual index_er cnt_hcc_grp rf_: hcc: 
		...
		%if &y.= readm_cnt or &y.= er_cnt or &y.= ins_los %then %do; 
				/include=7 dist=zinb; 
				zeromodel /*cbsa*/ spec_type index_setting index_cm age_grp dual index_er cnt_hcc_grp rf_: hcc: ;
		%end;
...
%do i=1 %to %sysfunc(countw(&sublst.));
	...
	%if &cnt.>500000 %then %do;
		proc sort data=model_&sub.; 
		by cbsa spec_type index_setting index_cm age_grp dual index_er;
		run;
		proc surveyselect data=model_&sub. method=srs n=500000 seed=1234 out=sample_&sub.;
...
left join meta.provider_x_cbsa_&py. b
