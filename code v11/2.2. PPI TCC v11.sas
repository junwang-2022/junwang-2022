********************************************;
*********** CJ PPI3 TCC Analysis ***********;
********************************************;

%let py=2017; /* set the performance year */

%let ccm_loc=D:\SASData\SAS_Shared_Data\LDS_CCM; /*put the location of the CCM files (soource data)*/
%let output_loc=D:\SASData\dua_052882\Sndbx\Jun_W\builder test\Builder v11\test v11\tcc;  /*put the location of the output data and log file*/
...
libname out "&output_loc.\py&py." compress=char; 
libname in "&output_loc.\py&py." compress=char; 
libname meta "&meta_loc" access=readonly;
libname meta2 "&meta2_loc" access=readonly;
libname npi "&npi_loc" access=readonly;

filename logfile "&output_loc.\py&py.\ppi_tcc_&sysdate9..log";
filename listfile "&output_loc.\py&py.\ppi_tcc_&sysdate9..list";
...
from pqem a
left join npi.npi_x_spec_&py. b
...
left join npi.provider_x_cbsa_&py. b3 on b3.provider_type="phys_npi" and b1.npi=b3.provider_id
left join npi.npi_x_spec_&py. b4 on b1.npi^="" and b1.npi=b4.npi
...
proc datasets lib=work noprint; delete readm_&epi. er_&epi. ; run; quit;
...
from out.bene_p_&epi.(drop=spec_cd spec_type) a
left join npi.npi_x_spec_&py. b on a.attr_npi=b.npi
...
set out.r_sc1_&prvdr._&epi. %if &enroll_type.^=COMM %then %do; out.r_sc2_&prvdr._&epi. %end;;
run;
