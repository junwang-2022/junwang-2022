...
left join npi.provider_x_cbsa_&py. b
...
left join range_&prvdr._&epi. c on c.rank_oe_adj=3 and c.max_oe<1 and a.&type.=c.&type. and a.spec_type=c.spec_type and a.lvl_1_value=c.lvl_1_value and a.measure=c.measure
...
if flag=1 then do;
	...
	if change^=0 then do;
		...
		else if avg_oe_adj>1.1 and rank_oe_adj^=0 then rerank_oe_adj=1;
	end;
end;
if 0.99<avg_oe_adj<1.01 then rerank_oe_adj=2;
