...
libname out "C:\Users\jun.wang\PPI3.0\def doc";
...
set meta.def_list;
where def_name="PPI_PERINATAL";

length epi_type $50. epi_length $10. attr_provider $100. clinical_category $50.;

epi_type="Perinatal";
epi_length="11 months";
attr_provider="Trigger operating/attending NPI";
episode_category="tx";
clinical_category="Pregnancy, childbirth and the puerperium";
...
data out.ppi_list;
set out.ppi_list perinatal;
run;

proc sql;
create table perinatal_drg as
select a.def_name, a.def_sub, "MS-DRG" as code_type length=10, b.ms_drg as code length=12 format=$12., b.ms_drg_desc as code_description length=100
from meta.def_list(where=(def_name="PPI_PERINATAL")) a
left join meta.ms_drg b on b.ppi_perinatal in ("C" "V")
order by def_sub, code;
quit;
proc sql;
create table perinatal_dx as
select a.def_name, a.def_sub, "ICD_10_DX" as code_type length=10, b.dx7 as code length=12 format=$12., b.dx7_desc as code_description length=100
from meta.def_list(where=(def_name="PPI_PERINATAL")) a
left join meta.icd_10_dx b on index(b.ppi_perinatal,"A")>0
order by def_sub, code;
quit;
proc sql;
create table perinatal_cpt as
select a.def_name, a.def_sub, "HCPCS/CPT" as code_type length=10, b.cpt as code length=12 format=$12., b.cpt_desc as code_description length=100
from meta.def_list(where=(def_name="PPI_PERINATAL")) a
left join meta.cpt b on b.ppi_perinatal in ("C" "V")
order by def_sub, code;
quit;

data trigger_perinatal;
set perinatal_drg perinatal_dx perinatal_cpt;
if code="" then delete;
run;

proc sql;
create table perinatal_trigger as
select b.episode_category, b.epi_type, b.clinical_category, b.epi_id, b.epi_name, a.code_type, a.code, a.code_description
from trigger_perinatal a
left join out.ppi_list b
on b.epi_id=a.def_name
order by episode_category, epi_type, clinical_category, epi_id,epi_name, code_type, code;
run;

data out.ppi_def_trigger;
set out.ppi_def_trigger perinatal_trigger;
run;
proc export data=out.ppi_list outfile="C:\Users\jun.wang\PPI3.0\def doc\ppi_list.csv" replace; run;
proc export data=out.ppi_def_trigger outfile="C:\Users\jun.wang\PPI3.0\def doc\ppi_def_trigger.csv" replace; run;
