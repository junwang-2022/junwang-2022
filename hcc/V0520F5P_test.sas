...
/*-> 28M*/
/*data person_20230516;*/
/*set eci00001.person_20230516(obs=7000000);*/
/*run;*/

*******************************************************************************;
* This distribution is:       V0520.128.Q3, March 2021
*******************************************************************************;
...
LIBNAME  LIBRARY "D:\SASData\dua_052882\Sndbx\ming\hcc";
FILENAME IN0     "D:\SASData\dua_052882\Sndbx\ming\hcc"; 
LIBNAME  IN1     "D:\SASData\dua_052882\Sndbx\ming\hcc\";
LIBNAME  IN2     "D:\SASData\dua_052882\Sndbx\ming\hcc\";
LIBNAME  IN3     "D:\SASData\dua_052882\Sndbx\ming\hcc\";
LIBNAME  IN4     "D:\SASData\dua_052882\Sndbx\ming\hcc\";
LIBNAME  INCOEF  "D:\SASData\dua_052882\Sndbx\ming\hcc\";
LIBNAME  OUT     "D:\SASData\dua_052882\Sndbx\ming\hcc";
 ...
 ***********************************************************************
 * 
 * Program V0520F5P creates 128 0/1 payment model HHS HCC variables,
 ...
 *
 *  V0520F5P passes parameters to main macro V0520F5M, which calls various internal and
 *    external macros. (A macro is a program fragment.) External macros are:
 *  I0V05ED4 - ICD10 edits based on age and/or sex
 *  AGESEXV6 - creates age/sex variables based on AGE_LAST
 *  V05128L1 - assigns labels to HHS HCCs and RXCs
 *  V05128H1 - sets selected HHS HCCs to zero based on hierarchical relationships
 ...
 *  CCFMT0Y1 - pointer to SAS formats that crosswalk ICD10s to HHS CCs for fiscal year 2020
 *  CCFMT0Y2 - pointer to SAS formats that crosswalk ICD10s to HHS CCs for fiscal year 2021 
 *  RXCFMTN  - pointer to SAS formats that crosswalk NDC to RXC for calendar year 2020
 *  RXCFMTH  - pointer to SAS formats that crosswalk HCPCS to RXC for calendar year 2020
...
%inc IN0("V0520F5M.SAS") / source2;

**==========================================================================================**;
** V0520F5M goes here when testing macros inline;
**==========================================================================================**;
...
run;
	...
	data diagnosis_20230516_03;
	set eci00001.diagnosis_20230516_dd(rename=(diagnosis_service_date=diagnosis_service_date_));
	diagnosis_service_date = input(put(diagnosis_service_date_, yymmddn8.), 8.);
    format diagnosis_service_date 8.;
	if substr(sysid,1,2) in ('03');
	keep sysid diag diagnosis_service_date;
	run;

	data person_20230516_03;
	set eci00001.person_20230516(rename=(dob=dob_));
	dob = input(put(dob_, yymmddn8.), 8.);
    format dob 8.;
	if substr(sysid,1,2) in ('03');
	keep sysid sex dob age_last metal csr_indicator enrolduration;
	run;

%V0520F5M( INP      = person_20230516_03,
           IND      = diagnosis_20230516_03,
           INN      = NDC,
           INH      = HCPCS,
           OUTDATA  = eci00001.person_cc_20230516_03,
           IDVAR    = sysid,
           KEEPVAR  = DOB
                      AGE_LAST
                      SEX
                      ...
                      CSR_ADJ_SCR_:
                      /*_ALL_*/,
           CCFMT0Y1 = HHS_V&CV.FY&YY1.P128C,
           CCFMT0Y2 = HHS_V&CV.FY&YY2.P128C,
           RXCFMTN  = NDCV&YY1.12_RXCV6_4F,
           RXCFMTH  = HCPC&YY1.12_RXCV6_4F,
